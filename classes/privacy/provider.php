...
namespace assignfeedback_writeup\privacy;
...
class provider implements
    ...
    public static function get_metadata(collection $collection) : collection {
        ...
        $collection->add_database_table('assignfeedback_writeup', $data, 'privacy:metadata:tablesummary');
        return $collection;
    }
    ...
    public static function export_feedback_user_data(assign_plugin_request_data $exportdata) {
        ...
        $plugin = $assign->get_plugin_by_type('assignfeedback', 'writeup');
        $writeup = $plugin->get_feedback_writeup($exportdata->get_pluginobject()->id);
        if ($writeup && !empty($writeup->commenttext)) {
            $data = (object)['commenttext' => format_text($writeup->commenttext, $writeup->commentformat,
                    ['context' => $exportdata->get_context()])];
            writer::with_context($exportdata->get_context())
                    ->export_data(array_merge($exportdata->get_subcontext(),
                            [get_string('privacy:commentpath', 'assignfeedback_writeup')]), $data);
        }
    }
    ...
    public static function delete_feedback_for_grade(assign_plugin_request_data $requestdata) {
        global $DB;
        $DB->delete_records('assignfeedback_writeup', ['assignment' => $requestdata->get_assignid(),
    ...
    public static function delete_feedback_for_grades(assign_plugin_request_data $deletedata) {
        ...
        $DB->delete_records_select('assignfeedback_writeup', "assignment = :assignment AND grade $sql", $params);
    }
}
